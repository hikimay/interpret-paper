...
`/interpret <URL> --detail <level>` で解説の詳細度を指定できる（デフォルト: `medium`）．

| level | 目安 | 定理・証明 | アルゴリズム |
|---|---|---|---|
| `low` | 各セクション 1〜3 段落 | 列挙のみ，説明最小限 | 概要のみ |
| `medium` | 各セクションを段落で解説 | 意義 + 証明のアイデアを 2〜4 段落 | 全ステップを説明 |
| `high` | 各セクションを最大限に詳細解説 | 証明を展開，数式変形のステップを追う，背景・比較・具体例を含む | 各行を詳細に説明 |
